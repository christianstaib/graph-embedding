...
[ -z "${exp_name}" ] && exp_name="pcba_flag_ft_dropnode"
[ -z "${seed}" ] && seed="1"
...
[ -z "${ckpt_path}" ] && ckpt_path=" " #checkpoint path
...
n_gpu=4 #$(nvidia-smi -L | wc -l)                   # Please use 4 GPUs (We use 4 V100 cards) to reproduce our results.
...
python ./graphormer/entry_nodedrop.py --num_workers 8 --seed $seed --batch_size $batch_size \
...
do
      ...
      echo "$file"
      echo -e "\n\n\n"
      python ./graphormer/entry_nodedrop.py --num_workers 8 --seed 1 --batch_size $batch_size \
            --dataset_name ogbg-molpcba \
