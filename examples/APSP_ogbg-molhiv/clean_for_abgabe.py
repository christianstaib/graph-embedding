...
def write_output(results):
    ...
    with open('data/rows.cor', 'w') as f_rows, open('data/shortest_paths.cor', 'w') as f_paths:
...
def write_paths_to_file(dataset, mode):
    ...
    with open('data/rows.cor', mode) as f_rows, open('data/shortest_paths.cor', mode) as f_paths:
        with Pool(multiprocessing.cpu_count()) as pool:
            for paths in tqdm(pool.imap(process_dict_label_tuple, dataset), total=len(dataset)):
                for path_str in paths:
...
if __name__ == '__main__':
    ...
    print(
        f'training took {int(time.time() - start)}s for {len(dataset)} graphs')
